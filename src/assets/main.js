...
const API_FAVORITES='https://api.thedogapi.com/v1/favourites?limit=50&api_key=live_o9FGwQ98uxzojWMh4aAkSI9bjnx4faI0Wea3ioIYemROoeuNU1ORh3QaXufkGngd'
...
async function fetch_dog(url){
    ...
    if (respose.status !==200){
        ...
        btng1.addEventListener('click',async()=>{await fetch_post_dog(API_FAVORITES,data[0].id)})
        btng2.addEventListener('click',async()=>{await fetch_post_dog(API_FAVORITES,data[1].id)})
        btng3.addEventListener('click',async()=>{await fetch_post_dog(API_FAVORITES,data[2].id)})
        btng4.addEventListener('click',async()=>{await fetch_post_dog(API_FAVORITES,data[3].id)})
    }

}


async function fetch_post_dog(url,id){
